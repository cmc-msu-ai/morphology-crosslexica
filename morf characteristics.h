...
public:
	...
	char* getPointer() {
		const int size = charSizes[int(characteristics[0])];
		char* res = new char[size];
		for (int i = 0; i < size; ++i) {
			res[i] = characteristics[i];
		}
		return res;
	}
	...
	void setNumber(int num) {
		...
		if (unsigned(characteristics[0]) == NOUN) {
			if (num == 1) {
				...
				characteristics[4] = char((2 << 4) | (4 & 15));
			}
	...
	void setGender(int gen) {
		...
		if (unsigned(characteristics[0]) == NOUN) {
			if (gen == 4) {
				characteristics[4] = char(((unsigned(characteristics[4]) >> 4) << 4) | (4 & 15));
			} else {
				characteristics[4] = char((1 << 4) | (gen & 15));
			}
		} else if (unsigned(characteristics[0]) == ADJECTIVE) {
	...
	bool isTranslatableTo(const MorfCharacteristics& mc) const;
