...
void MorfAnalizer::analize(const string& word, vector<void*>* result) const {
	vector<string> suffixes;
	aut.allSuffixes(append(word, sentinel), &suffixes);
	result->reserve(suffixes.size());
	for (int i = 0; i < (int)suffixes.size(); ++i) {
		result->push_back(reinterpret_cast<void*>(new MorfCharacteristics(suffixes[i])));
	}
}
...
string MorfAnalizer::synthesizeWithFullInformation(const string& word, const MorfCharacteristics& current, const MorfCharacteristics& needed) const {
	if (current.getPartOfSpeech() != needed.getPartOfSpeech()) {
		return string();
	}
...
void MorfAnalizer::synthesize(const string& word, const MorfCharacteristics& current, const MorfCharacteristics& needed, vector<vector<string> >* res) const {
	res->clear();
	if (current.getPartOfSpeech() != needed.getPartOfSpeech()) {
		return;
	}
	...
	for (int i = 0; i < allVariants.size(); ++i) {
		if (allVariants[i].satisfiesMask(needed)) {
			res->push_back(vector<string>());
			vector<string>& resEnd = res->at(res->size() - 1);
			MorfCharacteristics buff(needed);
			buff.copyConstantCharacteristics(allVariants[i]);
			vector<MorfCharacteristics> variants = buff.possibleCharacteristicsSatisfiingMask();
			for (int j = 0; j < variants.size(); ++j) {
				resEnd.push_back(synthesizeWithFullInformation(word, allVariants[i], variants[j]));
			}
		}
	}
}